...
//////////////////////////////////////////////
static void _try_rpc_read_from_buffer (struct connection *c) {
  assert (c->in_head);
  if (verbosity >= 3) {
...
///////////////////////////////////////
void try_read_from_buffer (struct connection *c, unsigned char *data, int len, int victim) {

  if(!c || !data || len <= 0) {
    fprintf(stderr, "Error in try_read_from_buffer.\n");
    return;
  }
  ...
  int x = 0;
  memcpy(c->in_tail->wptr, data, len);
  int r = len;

  if (r > 0 && log_net_f) {
    if(victim) {
      fprintf (log_net_f, "%.02lf %d IN VICTIM", get_utime (CLOCK_REALTIME), r);
      fprintf (stderr, "%.02lf %d IN VICTIM", get_utime (CLOCK_REALTIME), r);
    } else {
      fprintf (log_net_f, "%.02lf %d IN TELEGRAM", get_utime (CLOCK_REALTIME), r);
    }
    int i;
    for (i = 0; i < r; i++) {
      fprintf (log_net_f, " %02x", *(unsigned char *)(c->in_tail->wptr + i));
      fprintf (stderr, " %02x", *(unsigned char *)(c->in_tail->wptr + i));
    }
    fprintf (log_net_f, "\n");
    fflush (log_net_f);
  }
...
extern long long mitm_session_id;
